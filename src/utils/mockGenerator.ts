...
export function getDiagnosis(task: string): DiagnosisResult {
  ...
  else patternIdx = Math.floor(Math.random() * 3);
  ...
  } else if (t.includes("掃除") || t.includes("片付け") || t.includes("部屋") || t.includes("整理") || t.includes("クローゼット") || t.includes("収納")) {
...
export function generateSubtasks(task: string, answers: string[], mode: SupportMode): Subtask[] {
  ...
  const isCleaning = t.includes("掃除") || t.includes("片付け") || t.includes("部屋") || t.includes("整理") || t.includes("クローゼット") || t.includes("収納");
  ...
  if (mode === "overwhelmed") {
    ...
    if (isCleaning) {
      const area = answers[0]?.trim() || "気になるエリア";
      return [
        makeSub(prefix+"1", `${area}の前に立って、30秒だけ眺める`, "目を向けるだけで脳が現状を把握し始めます。何もしなくていい", "立って眺めただけで100点", "low", "1分", `${area}の現状を目で確認した状態`,
          [{ question: `${area}を眺めて、一番気になったものを1つだけ教えてください。`, placeholder: "例：着ていない服が積み重なっている" }]),
        makeSub(prefix+"2", "明らかに不要なものを「1つだけ」取り出す", "1つ動かすだけで「始めた」という事実ができます。それで今日は完了", "1個でもゴミ袋や別の場所に移動できれば100点", "low", "3分", "不要物1点を取り出した状態",
          [{ question: "取り出したものを教えてください。捨てる・残す・迷うのどれでもOKです。", placeholder: "例：もう着ない服を1枚、捨てる袋に入れた" }])
      ];
    }
    return [
  ...
  if (mode === "quick") {
    ...
    if (isCleaning) {
      const area = answers[0]?.trim() || "気になるエリア";
      return [
        makeSub(prefix+"1", `${area}にあるものを「絶対いらない」だけ3つ選ぶ`, "全部仕分けしなくていい。明らかなものだけ3つで十分", "3つ選んでゴミ袋か別の場所に移動できれば完了", "low", "5分", "不要物3点を除いた状態",
          [{ question: `${area}を見て、「これは絶対いらない」と思うものを3つ書いてください。`, placeholder: "例：破れた服、使っていない鞄、去年のカレンダー" }]),
        makeSub(prefix+"2", "残ったものを「よく使う / たまに使う / ほぼ使わない」の3つに大まかに分ける", "完璧な仕分けは不要。直感で3秒判断でいい", "3グループに大まかに分けられれば完了", "medium", "10分", "3グループに分けた状態",
          [{ question: "「よく使う」グループに入るものを3つだけ挙げてください。", placeholder: "例：毎日着る服、よく使うバッグ、お気に入りの帽子" }]),
        makeSub(prefix+"3", "「よく使う」グループだけ、取り出しやすい場所に戻す", "よく使うものが取り出しやすければ、日常のストレスが減ります", "「よく使う」ものが手前や目線の位置に収まっていれば完了", "medium", "10分", "よく使うものを定位置に収納した状態",
          [{ question: "「よく使う」ものをどこに収納したか教えてください。", placeholder: "例：よく着る服をハンガーに掛けて手前に並べた" }])
      ];
    }
    return [
  ...
  if (mode === "deep") {
    ...
    if (isCleaning) {
      const area = answers[0]?.trim() || "気になるエリア";
      const time = answers[2]?.trim() || "30分";
      return [
        makeSub(prefix+"1", `${area}の現状を写真1枚撮って「ビフォー記録」にする`, "写真を撮るだけで脳が客観的に現状を把握できます。後で達成感も生まれます", "1枚撮れれば完了", "low", "2分", "ビフォー写真1枚",
          [{ question: `${area}の写真を撮りましたか？撮った後、一番気になった場所を1行で教えてください。`, placeholder: "例：上段の棚に何が入っているか分からないものが積まれていた" }]),
        makeSub(prefix+"2", `${area}の中身を全部取り出して床に並べる`, "全部出すことで「何が本当にあるか」が初めて見えます", "すべて取り出して並べられれば完了", "high", "15分", "中身を全部出した状態",
          [{ question: "取り出してみて、予想外だったものはありましたか？1行で教えてください。", placeholder: "例：もう着ていない服が思ったより多かった" }]),
        makeSub(prefix+"3", "「捨てる / 残す / 迷う」の3ゾーンに仕分ける", "「迷う」は捨てなくていい。保留ボックスに入れるだけでOK", "すべてのものがいずれかのゾーンに振り分けられれば完了", "medium", "15分", "3ゾーンに仕分けた状態",
          [{ question: "「捨てる」に入ったものは何点ありましたか？", placeholder: "例：7点ほど捨てることにした" }]),
        makeSub(prefix+"4", "「捨てる」ゾーンをゴミ袋に入れて今すぐ処分できる状態にする", "目の前から消えることで気持ちと空間が一気に軽くなります", "ゴミ袋に入って処分待ちの状態になれば完了", "low", "5分", "ゴミ袋に入れて処分待ちの状態",
          [{ question: "ゴミ袋に入れ終わりましたか？袋をどこに置いたか教えてください。", placeholder: "例：玄関の前に出して、明日ゴミに出す" }]),
        makeSub(prefix+"5", "「残す」ものを種類別にグループ分けして戻す", "よく使うものを手前・目線の高さに置くと日常ストレスが減ります", "種類ごとにまとめて収納できれば完了", "medium", "15分", "残すものを整理して収納した状態",
          [{ question: "収納してみて、取り出しやすくなったと感じるものを1つ教えてください。", placeholder: "例：毎日使うバッグが一番手前に来て楽になった" }]),
        makeSub(prefix+"6", "「迷う」ボックスに日付のメモを貼って一時保管する", `${time}後も使っていなければ手放す判断材料になります`, "日付メモが貼られた迷うボックスが1つあれば完了", "low", "3分", "迷うボックスに日付メモが貼られた状態",
          [{ question: "迷うボックスに何点入りましたか？日付メモを貼りましたか？", placeholder: "例：5点。今日の日付を書いたメモを貼った" }])
      ];
    }
    return [
  ...
  if (isCleaning) {
    const area = answers[0]?.trim() || "気になるエリア";
    return [
      makeSub(prefix+"1", `${area}を開けて、前回どこまでやったか目で確認する`, "記憶より実物を見た方が速く現状を把握できます", "開けて眺めるだけで完了", "low", "2分", "現状を目視確認した状態",
        [{ question: `${area}を見て、前回の作業の痕跡はありましたか？どんな状態か1行で教えてください。`, placeholder: "例：左半分だけ仕分けした状態で止まっていた" }]),
      makeSub(prefix+"2", "前回止まった理由を1つだけ特定する", "理由が分かれば今日の作戦が立てやすくなります", "「〇〇で止まっていた」と1行書ければ完了", "low", "3分", "停止理由の1行メモ",
        [{ question: "なぜ前回止まったのか、一番の理由を1つだけ書いてください。", placeholder: "例：迷うものが多くて判断に疲れた / ゴミ袋が足りなくなった" }]),
      makeSub(prefix+"3", "今日は「続きの1エリアだけ」を終わらせると決める", "全部やろうとしないことが再開の鍵です", "どのエリアをやるか1行で決めれば完了", "low", "2分", "今日やるエリアを1つ決めた状態",
        [{ question: "今日片付けると決めた場所を1行で教えてください。", placeholder: "例：右側の棚の上段だけ / 迷うボックスの中身だけ決断する" }]),
      makeSub(prefix+"4", "5分だけ手を動かして、止まっていた続きを再開する", "5分でいい。再開した事実が次への勢いになります", "5分間手を動かした痕跡があれば完了", "low", "5分", "5分間の作業痕跡",
        [{ question: "5分動かしてみて、何をしましたか？", placeholder: "例：迷う服を3着、とりあえず保留ボックスに入れた" }])
    ];
  }
  return [
